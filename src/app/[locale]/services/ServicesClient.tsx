...
───────────────────────────────────────────── */
function CategoryCard({ cat, index, dictGrid, dictCategory, locale }: { cat: typeof categories[0]; index: number; dictGrid: any; dictCategory: any; locale: SupportedLocale }) {
  ...
  const label = dictCategory?.label ?? cat.label;
  const tagline = dictCategory?.tagline ?? cat.tagline;
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <Link href={getLocalizedPath(`/services/${cat.id}`, locale)} className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0f172a] rounded-xl mb-6" style={{ ['--tw-ring-color' as string]: cat.color }}>
        ...
        <div className="mt-auto flex items-center justify-between pt-5" style={{ borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          >
            <span>{expanded ? (dictGrid?.collapseLabel ?? 'Collapse') : (dictGrid?.viewMoreLabel ?? 'View {n} More').replace('{n}', String(cat.items.length - PREVIEW_COUNT))}</span>
            <ChevronDown
              size={14}
              strokeWidth={2.5}
              aria-hidden="true"
...
/* ─────────────────────────────────────────────
   PAGE
───────────────────────────────────────────── */
export default function ServicesClient({ dict, locale }: { dict: any; locale: string }) {
  ...
  const statLabels: string[] = dict?.statsLabels ?? ['Solution Categories', 'Service Offerings', 'Projects Delivered', 'Enterprise Clients'];
  const statValues = ['12', '140+', '150+', '80+'];
  const trustIndicators: string[] = dict?.trustIndicators ?? ['NDA on request', 'Response in 24h', 'No commitment needed'];
  const trustIcons = [Lock, Zap, CheckCircle2];
    ...
    <main lang={locale}>
        ...
        <div className="container-custom relative z-10">
          <div className="text-center max-w-4xl mx-auto">
            ...
            >
              ...
              {dict?.heroBadge2 ?? '12 Solution Categories · 140+ Offerings'}
            </motion.div>
            ...
            >
              {dict?.hero?.titlePrefix ?? 'Every Software Solution'}
              <br />
              <span className="gradient-text">{dict?.hero?.titleGradient ?? 'Your Business Needs'}</span>
            </motion.h1>
            ...
            >
              {dict?.hero?.subtitle ?? 'From custom SaaS platforms and enterprise portals to AI automation and cloud infrastructure — Eryon AI engineers production-grade software for businesses that demand excellence.'}
            </motion.p>
            ...
            >
              <Link href={getLocalizedPath("/contact", locale as SupportedLocale)} className="btn-primary">
                {dict?.cta?.btnPrimary ?? 'Discuss Your Project →'}
              </Link>
              <a
                href="#categories"
              ...
              >
                {dict?.exploreAllAnchor ?? 'Explore All Services ↓'}
              </a>
            </motion.div>
          </div>
        </div>
      ...
      <section id="categories" className="section-pad relative overflow-hidden" style={{ background: '#0f172a' }}>
        ...
        <div className="container-custom relative z-10" ref={gridRef}>
          ...
          <motion.div
            ...
            transition={{ duration: 0.45 }}
          >
            <SectionBadge icon={Layers} label={dict?.gridBadge ?? 'What We Build'} />
            <h2
            ...
            >
              {dict?.gridTitlePrefix ?? 'Purpose-Built Solutions, '}{' '}
              <span style={{ background: 'linear-gradient(135deg, #0066ff, #00b4d8)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                {dict?.gridTitleGradient ?? 'Not Generic Services'}
              </span>
            </h2>
            <p className="text-base md:text-lg" style={{ color: '#64748b', lineHeight: 1.7 }}>
              {dict?.gridSubtitle ?? 'Every offering below is a real deliverable — not a buzzword. Click any card to explore the full scope of that category.'}
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {categories.map((cat, i) => (
              <CategoryCard key={cat.id} cat={cat} index={i} dictGrid={dict?.grid} dictCategory={dict?.categories?.[i]} locale={locale as SupportedLocale} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden py-24 md:py-32" style={{ background: '#0f172a' }} ref={ctaRef}>
        ...
        <div className="container-custom relative z-10 text-center max-w-3xl mx-auto">
          ...
          >
            {dict?.bottomBadge ?? "Don't See What You Need?"}
          </motion.div>
          ...
          >
            {dict?.bottomTitle ?? 'We Build Anything Your Business Requires'}
          </motion.h3>
          ...
          >
            {dict?.bottomSubtitle ?? "Our engineering team handles entirely bespoke requirements. Share your idea and we'll scope it out — no commitment needed."}
          </motion.p>
          ...
          >
            <MovingBorderButton href={getLocalizedPath("/contact", locale as SupportedLocale)}>
              {dict?.cta?.btnPrimary ?? 'Start a Conversation →'}
            </MovingBorderButton>

            <Link
              href={getLocalizedPath("/portfolio", locale as SupportedLocale)}
            ...
            >
              {dict?.cta?.btnSecondary ?? 'View Our Work'}
          ...
          >
            {trustIndicators.map((label, i) => {
              const ItemIcon = trustIcons[i];
              return (
                <span key={i} className="inline-flex items-center gap-1.5">
                  <ItemIcon size={13} strokeWidth={2.5} aria-hidden="true" />
                  {label}
                </span>
              );
            })}
