...
import { getRelatedService } from '@/lib/blog-service-map';
...
// Unicode-aware slug: \p{L}/\p{N} keep letters & digits from any script (Japanese,
// Arabic, etc.) instead of collapsing non-Latin text to empty/duplicate ids.
function slugify(text: string): string {
  return text.trim().toLowerCase().replace(/[^\p{L}\p{N}]+/gu, '-').replace(/(^-|-$)/g, '') || 'section';
}

// Extract headings from markdown for TOC. Ids are de-duplicated so repeated
// heading text (common across translated content) never collides.
...
export default function BlogDetailClient({ post, related }: BlogDetailClientProps) {
  ...
  // Rendered heading text can appear more than once (e.g. across sections);
  // walk the pre-computed, already-deduplicated `headings` list in order so
  // the ids the markdown renderer assigns always match the TOC's hrefs.
  const headingIdQueues = useRef(new Map<string, string[]>());
  headingIdQueues.current = new Map();
  for (const h of headings) {
    const list = headingIdQueues.current.get(h.text) ?? [];
    list.push(h.id);
    headingIdQueues.current.set(h.text, list);
  }
  function nextHeadingId(text: string): string {
    const list = headingIdQueues.current.get(text);
    return (list && list.shift()) || slugify(text);
  }
  ...
  // Built from known server data (locale + slug), not window.location, so the
  // server-rendered and hydrated client output always match.
  const shareUrl = `https://www.eryonai.com${getLocalizedPath(`/blogs/${post.slug}`, locale)}`;
          ...
          <div className="max-w-4xl mx-auto px-6 pb-14 w-full">
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                <Link href={getLocalizedPath("/blogs", locale)} className="text-blue-300 text-sm hover:text-white transition-colors">
                  ← {t.backToBlog}
                </Link>
        ...
        <div className="flex flex-col md:flex-row gap-12">
          ...
          <article className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-2.5 mb-10 pb-6" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
              <span className="text-sm font-semibold text-slate-400 mr-1 w-full sm:w-auto">{t.share}</span>
              <a
            ...
            <div className="max-w-none text-slate-300">
              <ReactMarkdown
                ...
                components={{
                  ...
                  h2: ({ children }) => {
                    const id = nextHeadingId(String(children));
            ...
            {relatedService && (
              <Link
                href={getLocalizedPath(`/services/${relatedService.slug}`, locale)}
                className="mt-6 p-6 rounded-2xl flex items-center justify-between gap-4 group transition-colors"
                style={{ background: 'rgba(0,102,255,0.06)', border: '1px solid rgba(0,102,255,0.18)' }}
              >
                <div>
                  <p className="text-xs font-bold tracking-widest uppercase mb-1" style={{ color: '#00b4d8' }}>{t.relatedService}</p>
                  <p className="font-bold text-slate-50" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{relatedService.label}</p>
                </div>
                <span className="text-blue-300 text-sm font-semibold whitespace-nowrap group-hover:text-white transition-colors">
                  {t.discussProject ?? 'Learn more'} →
                </span>
              </Link>
            )}
          </article>
            ...
            <aside className="hidden md:block md:w-56 lg:w-64 xl:w-72 shrink-0">
              <div className="sticky top-24">
                <GlowCard color="#0066ff" className="p-5" style={{ background: 'rgba(255,255,255,0.03)' }}>
                  <h3 className="text-xs font-bold tracking-widest text-slate-500 uppercase mb-4">{t.tableOfContents}</h3>
                ...
                <GlowCard color="#0066ff" className="p-5 mt-4" style={{ background: 'rgba(255,255,255,0.03)' }}>
                  <h3 className="text-xs font-bold tracking-widest text-slate-500 uppercase mb-4">{t.shareArticle}</h3>
