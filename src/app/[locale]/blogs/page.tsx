...
import { getLocaleFromPathname, getLocalizedPath, contentUI } from '@/lib/layout-translations';
import { getLocalizedPosts, getLocalizedPost, getCategoryLabel } from '@/lib/blog-translations';
...
export default function BlogsPage() {
  ...
  const featured = getLocalizedPost(getFeaturedBlog(), locale);
  const trending = getLocalizedPosts(getTrendingBlogs(4), locale);

  const filtered = localizedPosts.filter((p) => {
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between mb-6">
                <p className="text-sm text-slate-500">
                  ...
                  {activeCategory !== 'All' && <span> in <strong className="text-slate-400">{getCategoryLabel(activeCategory, locale)}</strong></span>}
                </p>
              </div>
                ...
                <GlowCard color="#f59e0b" className="p-6" style={{ background: 'rgba(255,255,255,0.03)' }}>
                  ...
                  <div className="flex flex-col gap-4">
                    {trending.map((post, i) => (
                      ...
                      >
                        <Link href={getLocalizedPath(`/blogs/${post.slug}`, locale)} className="flex gap-3 group items-start">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">{post.readTime} {t.minRead} · {getCategoryLabel(post.category, locale)}</p>
                          </div>
                        </Link>
                      </motion.div>
                    ))}
