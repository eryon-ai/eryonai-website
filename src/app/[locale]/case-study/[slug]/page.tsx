...
import { getLocalizedPath, contentUI, buildAlternates, SupportedLocale } from '@/lib/layout-translations';
...
const projects: Record<string, Project> = {
  ...
  'infra-erp': {
    slug: 'infra-erp',
    title: 'Construction AI ERP',
    subtitle: 'Enterprise Infrastructure Management Platform',
    category: 'Enterprise / Construction',
    color: '#00b4d8',
    tagline: 'A full-scale ERP powering an entire infrastructure operation — from AI drawing analysis to real-time payroll.',
    overview: 'We engineered a comprehensive, end-to-end ERP system for a leading infrastructure and construction firm. The platform unifies project management, workforce payroll, AI-driven floor plan estimation, Google Sheets ledger generation, contractor billing, attendance tracking, and multi-role access control into one cohesive system — replacing scattered spreadsheets and manual processes entirely.',
    challenge: 'The client was managing dozens of concurrent construction projects using disconnected tools — attendance in one spreadsheet, payroll in another, and bills tracked manually. There was no unified visibility into project financials, workforce costs, or material stock. They also needed an AI tool to generate cost estimates directly from uploaded architectural drawings (DWG/DXF files) without relying on external vendors.',
    solution: 'We built a full-stack platform using Next.js 14 (App Router) for the frontend and a Python FastAPI backend with PostgreSQL. The system features a role-based portal covering Admins, Site Engineers, Supervisors, Contractors, Consultants, Workers, and Clients. Key innovations include: an AI Estimator that analyzes DWG/DXF floor plans using vision models to generate BOQ and BBS reports priced against a published Schedule of Rates (SOR); automated Google Sheets ledger generation for client billing; presigned MinIO storage for all documents and drawings; and a real-time attendance and payroll engine with daily/monthly wage calculations.',
    metrics: [
      { label: 'Modules Built', value: '15+' },
      { label: 'User Roles', value: '8 Roles' },
      { label: 'AI Feature', value: 'DWG Estimator' },
      { label: 'Ledgers', value: 'Auto-Generated' },
    ],
  ...
  hirestream: {
    ...
    images: [
      'https://f005.backblazeb2.com/file/eryonaiWebsiteImages/hirestream-login.jpg',
      'https://f005.backblazeb2.com/file/eryonaiWebsiteImages/hirestream-candidate-dashboard.jpg',
      'https://f005.backblazeb2.com/file/eryonaiWebsiteImages/hirestream-admin-dashboard.jpg',
      'https://f005.backblazeb2.com/file/eryonaiWebsiteImages/hirestream-admin-overview.jpg',
    ],
  },
  kyprox: {
    slug: 'kyprox',
...
export default async function CaseStudyPage({
  ...
  const p = projects[slug];
  if (!p) notFound();
  const t = contentUI[locale] ?? contentUI.en;
  ...
  return (
    <div style={{ background: '#0f172a', minHeight: '100vh', fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <nav style={{
        ...
        <Link href={getLocalizedPath("/portfolio", locale)} className="group" style={{
          display: 'inline-flex',
          alignItems: 'center',
          gap: 8,
          color: '#0066ff',
      ...
      <section style={{ maxWidth: 1200, margin: '0 auto', padding: '0 24px 64px' }}>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-5 lg:gap-8 items-start">
          ...
          <div>
            {[
              { heading: t.projectOverview, body: p.overview },
              { heading: t.theChallenge, body: p.challenge },
              { heading: t.ourSolution, body: p.solution },
            ].map(({ heading, body }, i) => (
          ...
          <FadeIn delay={0.15} className="static lg:sticky lg:top-19">
            <div style={{
              ...
              <div style={{ marginTop: 28, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                ...
                <MovingBorderButton href={getLocalizedPath("/contact", locale)} className="px-6! py-2.5! text-[13px]">
                  {t.discussProject}
